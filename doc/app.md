...
*Plus small helpers: `isInlineFsVerb`, `moveInputSuggestion`, `moveSidebarSelection`, `moveQueryToParentDirectory` — suggestion plumbing and highlight movement. `moveSidebarSelection` and `moveQueryToParentDirectory` stop at the selected repo's edge with a status message, and snap a highlight that drifted outside (a mouse click or Ctrl+P open) back to the repo root; `findEntryIndex` does the root lookup.*
...
- `renderHeader` builds the top row: the version/root title plus, when a workspace repo is selected with Ctrl+W, a `working repo: x` label in the sidebar's uncommitted yellow. Styling the label apart from the title means the row is measured and padded by hand rather than through `Width()`, and a row wider than the terminal is cut from the title first so the repo name survives.
