...
`orderDirPrefix` implements the directory-browse rule. When the query contains a `/`, candidates under that literal prefix partition to the front via a single stable-partition pass — one case-folded prefix test per match, where a comparator-based sort would re-fold it O(n log n) times. With no typed tail (query ends in `/`) the in-class span then sorts like a file explorer — directories (trailing `/`) first, then files, each group alphabetical — and with a tail the in-class order stays score-ranked. Without this, every child of a directory scores identically and the listing degrades to path-length order.
