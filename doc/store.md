...
- `SaveCorpus` / `LoadCorpus` — the corpus is a singleton at a fixed key; each save overwrites the last. A large index (≥64 KiB of JSON) auto-offloads to ntee-db's blob side-file.
...
*Plus map-backed mirrors of the rest of the interface: `Close`, `TouchOpened`, `RecentFiles`, `DeleteOpenedUnder`, `SnapshotPut`, `SnapshotGet`, `SnapshotDelete`, `SaveSession`/`LoadSession`, `SaveDraft`/`LoadDraft`/`DeleteDraft`, `SaveTabs`/`LoadTabs`, `SaveCorpus`/`LoadCorpus` — same semantics as the real store, minus persistence.*
