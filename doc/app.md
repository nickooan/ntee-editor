...
- `New` constructs the Model: wires the store, LSP registry, and clipboard, restores the previous session's tabs (or last file) with their drafts, and warm-adopts the persisted search corpus so the first frame paints instantly even on a huge tree. A cold cache instead shows the splash and lets `Init` build the index in the background.
- `Init` fires the startup Cmds: the corpus build (cold) or a background signature re-validation of the adopted index (warm), the splash ticker, and — in a git repo — the first status refresh plus the poll loop.
...
- `highlightedEntryIndex` resolves that highlight to a row. With a working repo selected, text *typed* into the bar goes through `repoScopedHighlight`: an exact path or name inside the repo, then an exact full path anywhere (honored outside the repo, with the warning), then prefix/substring matches inside the repo, then the nearest in-repo ancestor and the repo root — so after a Shift+arrow walk outside, typing brings the highlight back to the repo's matching file. Finder and keyboard navigation, and a confirmed selection with an empty bar, resolve as-is.
- `outsideRepoWarning` derives the query status line's orange `outside repo x · read-only` warning from the current highlight every frame, so it persists exactly as long as the highlight sits outside the working repo.
- `openFileAt` is the one deliberate-open path: stat-first (a missing file errors instead of becoming a tab), stashes the outgoing buffer as a draft, closes/opens LSP documents, clears any diff/conflict/preview state, starts a fresh edit session, restores a stashed draft and the remembered cursor, and lands in edit mode. A path outside the Ctrl+W-selected repo (`isReadOnlyPath`) instead lands in the view pane: its stored draft is left untouched (the tab stays red and the edits return when the scope widens) and the status line tags the file read-only.
- `isReadOnlyPath` is the single "outside the selected workspace repo" predicate — false when the opened directory is itself a repo, when the whole workspace is selected, or when no file is open.
- `gitScopeFor` is the read counterpart for running git on a file: it returns the folder to run git in and the path relative to that repo. When the opened directory is itself a repo it returns the root and the unchanged path; in a workspace it picks the deepest nested repo containing the file. An empty repo list that has not been scanned yet reports `loading` so callers can say "still loading" instead of "not a repository". It reads only the in-memory repo list, never the disk. `repoRelativePath` wraps it for the copy commands, and `gitScopeError` maps the failure to its status message.
...
*Plus small helpers: `isInlineFsVerb`, `moveInputSuggestion`, `moveSidebarSelection`, `moveQueryToParentDirectory` — suggestion plumbing and highlight movement. Neither `moveSidebarSelection` (Shift+↑/↓) nor `moveQueryToParentDirectory` (Esc) is bounded by the working repo: both may leave it, and `outsideRepoWarning` flags that in orange; `findEntryIndex` looks up a row by path.*
...
- `handleSearchKey` types the query, cycles matches with ↑/↓, opens the replace bar with Ctrl+E, and commits with Enter. Ctrl+E is refused with a status message when the search was entered from the read-only view (`searchPrevMode != modeEdit`), since replace needs an edit session.
...
- `jumpBack` (Ctrl+O) pops the trail; a frame that originated inside a diff or blame view *re-enters* that view, re-deriving it (the buffer may have changed at the jump target) with the pending position restored when the recomputed rows land — unless the return target has become read-only (its scope was narrowed), in which case it lands in the view pane instead. It is reachable from the view pane too, so a Ctrl+J that landed in an outside-repo file is not a dead end.
- `openJumpFile` is `openFileAt`'s jump-flavored sibling: same draft-stash/restore guarantees, but it keeps the jump stack intact and lands at an exact position. A read-only (outside-repo) target keeps the trail and lands in the view pane with its draft left untouched.
...
- `renderHeader` builds the top row: the version/root title plus, when a workspace repo is selected with Ctrl+W, a `working repo: x` label in the same orange as the outside-repo warning. Styling the label apart from the title means the row is measured and padded by hand rather than through `Width()`, and a row wider than the terminal is cut from the title first so the repo name survives.
