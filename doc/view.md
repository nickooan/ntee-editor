...
- `BuildMatchesByLine(matches)` — buckets matches into a `map[line][]LineMatch`, tagging each with its global match index (so the renderer can style the focused match differently) and sorting each bucket by start offset.

### viewport.go

- `NormalizeLines(content)` — splits content on `\n`, treating CRLF as a line break too, so a stray `\r` never reaches the terminal.
