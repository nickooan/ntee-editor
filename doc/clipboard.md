...
- `Copy` — the package's public surface. Tries each native command in order: look up the binary, pipe the text to its stdin, and stop on the first success. Every spawn runs under a 3-second deadline (the same hard-timeout rule `gitcmd` applies to git) so a hung `wl-copy` or `xclip` can't freeze the caller. A tool that is present but fails or times out doesn't abort — the next candidate gets a shot, and OSC 52 is the final fallback.
